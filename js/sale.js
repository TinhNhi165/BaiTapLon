...
function ageRange() {
    let dropdownMenu = document.getElementById("ageRange");
...
function occasion() {
    let dropdownMenu = document.getElementById("occasion");
    if (dropdownMenu.style.display === "block") {
        dropdownMenu.style.display = "none";
    } else {
        dropdownMenu.style.display = "block";
...
function gender() {
    let dropdownMenu = document.getElementById("gender");
    if (dropdownMenu.style.display === "block") {
        dropdownMenu.style.display = "none";
    } else {
        dropdownMenu.style.display = "block";
...
function color() {
    let dropdownMenu = document.getElementById("color");
    if (dropdownMenu.style.display === "block") {
        dropdownMenu.style.display = "none";
    } else {
        dropdownMenu.style.display = "block";
...
function sleeveLength() {
    let dropdownMenu = document.getElementById("sleeveLength");
    if (dropdownMenu.style.display === "block") {
        dropdownMenu.style.display = "none";
    } else {
        dropdownMenu.style.display = "block";
...
let money = 100;
function price() {
    let dropdownMenu = document.getElementById("price");
    if (dropdownMenu.style.display === "block") {
        dropdownMenu.style.display = "none";
    } else {
        inputRange();
...
function inputRange() {
    ...
    let output = document.getElementById("priceChange");
...
function hasCommonElement(arr1, arr2) {
    // Lặp qua từng phần tử của mảng thứ nhất
    for (let i = 0; i < arr1.length; i++) {
        // Kiểm tra xem phần tử hiện tại của mảng thứ nhất có tồn tại trong mảng thứ hai không
        if (arr2.includes(arr1[i])) {
            return true; // Nếu có ít nhất một phần tử chung, trả về true
        }
    }
    return false;
}
...
function renderProduct() {
    ...
    if (checkedBoxes.length === 0) {
        cart.forEach(function (element) {
            let product = document.getElementById(String(element.id));
        ...
        cart.forEach(function (element) {
            let product = document.getElementById(String(element.id));
            if (element.price <= money && hasCommonElement(checkedBoxes, element.selectionCriteria)) {
                product.style.display = "block";
            } else {
                product.style.display = "none";
            }
...
let storedData = localStorage.getItem('searchInput');
if (storedData) {
    searchInput.value = storedData;
    console.log("ok");
    handleInputChange(); // Gọi hàm xử lý ngay sau khi gán giá trị mới
}
...
function handleInputChange() {
    ...
    const sp = {
        "PRIMARY": ["0", "4", "8", "9", "10", "11", "12", "13", "14"],
        "PEEK KIDS": ["1"],
        "MANIERE": ["2", "3"],
        "PETITE PLUME": ["5"],
        "BENBEN": ["6"],
    ...
    if (searchTerm.trim() === "") {
        ...
        for (let i = 0; i < 15; i++) {
            let id = i.toString();
            let product = document.getElementById(id);
            if (sp.hasOwnProperty(searchTerm)) {
                if (sp[searchTerm].includes(id)) {
                    product.style.display = "block";
                } else {
                    product.style.display = "none";
                }
